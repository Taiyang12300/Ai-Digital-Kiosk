...
function cleanTextForSpeech(text) {
    return text
        ...
        .replace(/[*#_]/g, '')
        .replace(/\-\-+/g, '')
        // เวลา เช่น 08.00 น. → เวลา 8 นาฬิกา 00 นาที
        .replace(/(?<!เวลา\s*)(\d{1,2})\.00\s*น\./g, 'เวลา $1 นาฬิกา')
.replace(/(?<!เวลา\s*)(\d{1,2})\.(\d{2})\s*น\./g, 'เวลา $1 นาฬิกา $2 นาที')
        // คำย่อกฎหมาย
        ...
        .replace(/บ\.(\d+)/g, 'บ $1')
        .replace(/ท\.(\d+)/g, 'ท $1')
        // หัวข้อ "1. ยื่น" → "1 ยื่น" (ป้องกัน Pattara สะดุด)
        .replace(/(\d+)\.\s/g, '$1 ')
        // วงเล็บ → เว้นวรรค
        .replace(/[()[\]]/g, ' ')
        // ช่องว่างซ้อน
